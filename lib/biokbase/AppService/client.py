...
class AppService(object):
    ...
    def service_status(self):
        resp = self._call('AppService.service_status',
                          [])
        return resp[0]

    def enumerate_apps(self):
        resp = self._call('AppService.enumerate_apps',
                          [])
        return resp[0]

    def start_app(self, app_id, params, workspace):
        resp = self._call('AppService.start_app',
                          [app_id, params, workspace])
        return resp[0]

    def start_app2(self, app_id, params, start_params):
        resp = self._call('AppService.start_app2',
                          [app_id, params, start_params])
        return resp[0]

    def query_tasks(self, task_ids):
        resp = self._call('AppService.query_tasks',
                          [task_ids])
        return resp[0]

    def query_task_summary(self):
        resp = self._call('AppService.query_task_summary',
                          [])
        return resp[0]

    def query_task_details(self, task_id):
        resp = self._call('AppService.query_task_details',
                          [task_id])
        return resp[0]

    def enumerate_tasks(self, offset, count):
        resp = self._call('AppService.enumerate_tasks',
                          [offset, count])
        return resp[0]

    def kill_task(self, id):
        resp = self._call('AppService.kill_task',
                          [id])
        return resp

    def rerun_task(self, id):
        resp = self._call('AppService.rerun_task',
                          [id])
        return resp[0]
