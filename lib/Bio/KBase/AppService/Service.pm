package Bio::KBase::AppService::Service;
...
sub call_method {
    ...
    my $ctx = Bio::KBase::AppService::ServiceContext->new(client_ip => $self->getIPAddress());
    $ctx->module($modname);
    $ctx->method($method);
...

{
	...
	{
	    $self->exception('PerlError', "Authentication required for AppService but no authentication header was passed");
	}
